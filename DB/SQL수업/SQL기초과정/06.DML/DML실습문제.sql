...
*/ 
UPDATE TB_MEMBER SET PAY_CARD_NO = '1234-5678-AAAA-BBBB'
...
*/
UPDATE TB_MEMBER SET GRADE_CD = 2 WHERE GRADE_CD =1;
COMMIT;
...
UPDATE TB_MEMBER_TEL SET TEL_NO = '010-9999-9999' WHERE MEMBER_ID = 'BBBBB';
COMMIT;
...
8. TB_MEMBER_LIKE 테이블은 회원이 선호하는 상품타입을 등록해놓은 테이블입니다. 
   현재 BBBBB가 선호하는 상품은 다음과 같습니다. (SELECT * FROM TB_MEMBER_LIKE WHERE MEMBER_ID = 'BBBBB' ) 
   [ 컴퓨터 , 스마트폰 ] 
...
UPDATE TB_PRD SET SELL_COMP_NAME = '삼성ELEC' 
WHERE SELL_COMP_NAME = '삼성'
AND PRD_TYPE AND (컬럼명 = '컴퓨터' OR 컬럼명 = '스마트폰');
COMMIT;
