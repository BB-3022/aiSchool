 ...
 <!-- mypage START -->
  <div class="container">
    ...
    <form action="EditService" method="post" id="editForm">
        ...
        <input type="text" class="form-control custom-input" name="mb_id" id="mb_id" value="<%=info.getMb_id() %>" readonly>
   <!-- 현정추가 끝 -->
        ...
        <input type="text" class="form-control custom-input" name="mb_birth">
         ...
         <div class="card-body">
            <div class="css-mypagebutton">
               <button class="css-ufulao e4nu7ef3" type="submit" formaction="DeleteService" style="background-color:#faa15c;">
                  <span class="btn css-nytqmg e4nu7ef1" style="color:white;">탈퇴하기</span>
               </button>
               <button class="css-1qirdbn e4nu7ef3" type="submit" formaction="EditService" style="background-color:#faa15c;">
                  <span class="css-nytqmg e4nu7ef1" style="color:white;">회원정보수정</span>
               </button>
            </div>
         ...
         <div class="WISHLIST">
            ...
            <div>
               <a class="deleteAll" href="WishprdAlldelete" style="background-color:#faa15c; margin-left: 85%">전체삭제</a>
            </div>
            <br>
...
<script>
   ...
   // 탈퇴하기 버튼 클릭 시 알림 창 표시
    document.querySelector('#deleteButton').addEventListener('click', function () {
      if (confirm('회원 정보가 모두 사라지게 됩니다. 탈퇴하시겠습니까?')) {
         document.getElementById('editForm').setAttribute('action', 'DeleteService');
         document.getElementById('editForm').submit();
      }
   });
</script>
